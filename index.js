...
const mariadbVersion = parseFloat(
  process.env["INPUT_MARIADB-VERSION"] || "10.9"
).toFixed(1);

if (
  !["10.9", "10.8", "10.7", "10.6", "10.5", "10.4", "10.3"].includes(
    mariadbVersion
  )
) {
...
if (process.platform == "darwin") {
  ...
  run(
    `curl -Ls -o mariadb.msi https://downloads.mariadb.com/MariaDB/mariadb-${fullVersion}/winx64-packages/mariadb-${fullVersion}-winx64.msi`
  );
  ...
  run(
    `echo "deb http://downloads.mariadb.com/MariaDB/mariadb-${mariadbVersion}/repo/ubuntu $(. /etc/os-release && echo $VERSION_CODENAME) main" | sudo tee /etc/apt/sources.list.d/mariadb.list`
  );
  run(
